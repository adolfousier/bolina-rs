...
| 2026-09-06 | sweep | Warning sweep pré-wiring (convergência de trabalho staged da sessão 09:12): `#![allow(dead_code)]` provisório em 25 módulos — marcado "remove post-parity" em lib.rs/main.rs, API W7-W11 ainda não ligada ao daemon e `deny(warnings)` partia o build; re-exports mac1/noise removidos de transport/mod.rs, testes importam directo dos submódulos, imports mortos limpos. Suite: **373 passed / 0 failed** (crescimento 319→373 pelos testes W10/W11). Drift pré-existente em HEAD documentado sem tocar: cargo fmt 58 ficheiros (toolchain 1.98, repo nunca teve gate fmt) + clippy 41 lints de estilo sob deny(warnings) — ambos anteriores a este sweep, decisão de gate pendente. |
...
## 2026-09-12 — counters catch what 19 h of soaks could not: unframed bindings, ~100 pkt/s ceiling; G4 re-run required

**Found (counters landed `b859732`, then measured within seconds):** (1) at `9a1cdf1` the ladder A binding was built inline WITHOUT `u16be(cert_len)`; the daemon parses strict → dropped silently → A bound nothing and every A envelope was lost in all 25,689 G4 rounds. Verified at the sealed commit: B (`ladder_b.rs:74`) and C (`ladder_c.rs:44`) use the framed `open_bound_session` — intact; D is HTTP — unaffected. G4 lost 1 of 4 ladders, and its own w12 rig used the correct frame, so daemon-and-test agreed while the client was the odd one out — the rung-E symmetry class, on our side. (2) Daemon ingest ceiling ≈ 100 pkt/s: `run_loop` drains one packet per 10 ms tick; V's 2 000-envelope bursts sat in the kernel buffer — the flat G5 latency curve measured socket absorb, not the daemon. Corrected estimates: G4 integrated path = B/C/D; G5 wire arrivals ≈ 3/round (~36 k, not millions). Both receipts carry dated Correction sections; G4 (which sustains the seal) must be re-run instrumented before any signature.
