...
fn be_mesh_02_parse_relay_registration_non_zero_reserved() {
    ...
    buf[2] = 1;
    assert_eq!(
        RelayRegistration::parse(&buf),
        Err(RelayError::NonZeroReserved)
    );
}
...
fn be_mesh_02_relay_table_prunes_expired() {
    let mut t = RelayTable::new();
    let mut addr1 = [0u8; LEN_OVERLAY_ADDR];
    addr1[0] = 1;
    let mut addr2 = [0u8; LEN_OVERLAY_ADDR];
    addr2[0] = 2;
    t.insert(RelayEntry {
        overlay_addr: addr1,
        relay_index: 0,
...
fn be_mesh_02_forward_packet_stale_route() {
    ...
    assert_eq!(
        forward_packet(&t, &route, &packet, 2000),
        Err(RelayError::StaleRoute)
    );
}
...
fn be_mesh_02_forward_packet_unknown_recipient() {
    ...
    let packet = [1, 2, 3];
    assert_eq!(
        forward_packet(&t, &route, &packet, 500),
...
fn exact_max_expiry_must_parse() {
    ...
    use bolina::transport::relay::{RelayRegistration, LEN_RELAY_REGISTRATION, MAX_EXPIRY};
    let mut buf = [0u8; LEN_RELAY_REGISTRATION];
    ...
    assert!(
        reg.is_ok(),
        "expiry == MAX_EXPIRY must parse, got {:?}",
        reg.err()
    );
    ...
    buf[36..44].copy_from_slice(&(MAX_EXPIRY + 1).to_be_bytes());
    assert!(
        RelayRegistration::parse(&buf).is_err(),
        "expiry == MAX_EXPIRY+1 must fail"
    );
}
