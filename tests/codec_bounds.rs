...
fn cursor_u32be_sequential_reads_independent() {
    let e = Envelope {
        version: 2,
        channel_id: &[0u8; LEN_CHANNEL_ID],
        sender: &zero_pubkey(),
        seq: 0xAABBCCDD_11223344,
        parents: &[],
        parent_count: 0,
        ts: 0,
        body_type: 0x02,
...
fn envelope_body_len_oversized() {
    ...
    assert!(
        matches!(result, Err(ParseError::Oversize)),
        "body_len MAX_BODY+1 must fail, got {:?}",
        result
    );
}
...
fn envelope_parent_count_boundary() {
    ...
    let wire = encode_envelope(&e);
    assert!(
        parse_envelope(&wire).is_ok(),
        "parent_count == MAX_PARENTS must succeed"
    );
    ...
    assert!(
        matches!(result, Err(ParseError::Oversize)),
        "parent_count MAX_PARENTS+1 must fail, got {:?}",
        result
    );
}
...
fn cert_scope_count_boundary() {
    ...
    assert!(
        parse_cert(&wire).is_ok(),
        "scope_count == MAX_SCOPE must succeed, got {:?}",
        parse_cert(&wire)
    );
    ...
    assert!(
        matches!(result, Err(ParseError::Oversize)),
        "scope_count MAX_SCOPE+1 must fail Oversize, got {:?}",
        result
    );
}
...
fn cert_ca_sig_count_boundary() {
    ...
    assert!(
        parse_cert(&wire).is_ok(),
        "ca_sig_count == MAX_CA_SIGS must succeed, got {:?}",
        parse_cert(&wire)
    );
    ...
    assert!(
        matches!(result, Err(ParseError::Oversize)),
        "ca_sig_count MAX_CA_SIGS+1 must fail, got {:?}",
        result
    );
}
...
fn cert_ca_keys_equal_fails() {
    ...
    ca_sigs.extend_from_slice(&zero_sig());
    ca_sigs.extend_from_slice(&key);
    ca_sigs.extend_from_slice(&zero_sig()); // SAME key
    let c = Cert {
        version: 3,
        role_bits: 0x04,
        sig_pubkey: &[1u8; 32],
...
fn cert_ca_keys_descending_fails() {
    let mut key1 = [0u8; LEN_CA_KEY];
    key1[0] = 10;
    let mut key2 = [0u8; LEN_CA_KEY];
    key2[0] = 5;
    let mut ca_sigs = Vec::new();
    ca_sigs.extend_from_slice(&key1);
    ca_sigs.extend_from_slice(&zero_sig());
    ca_sigs.extend_from_slice(&key2);
    ca_sigs.extend_from_slice(&zero_sig());
    let c = Cert {
        version: 3,
        role_bits: 0x04,
    ...
    assert!(
        matches!(result, Err(ParseError::Malformed)),
        "descending CA keys must fail, got {:?}",
        result
    );
}
...
fn cert_name_at_max_name_boundary() {
    ...
    assert!(
        result.is_ok(),
        "name len == MAX_NAME must succeed, got {:?}",
        result
    );
}
...
fn envelope_body_at_max_body_boundary() {
    ...
    assert!(
        result.is_ok(),
        "body_len == MAX_BODY must succeed, got {:?}",
        result
    );
}
...
fn cert_ca_keys_ascending_must_succeed() {
    ...
    assert!(
        result.is_ok(),
        "ascending CA keys must succeed, got {:?}",
        result
    );
}
...
fn intent_action_at_max_action_boundary() {
    ...
    assert!(
        result.is_ok(),
        "action len == MAX_ACTION must succeed, got {:?}",
        result
    );
}
