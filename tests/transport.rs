...
fn msg1_layout_byte_level_matches_spec_41a() {
    ...
    assert_eq!(
        &msg[4..8],
        &0x51530001u32.to_be_bytes(),
        "sender index big-endian"
    );
    ...
    assert!(
        verify_mac1(&r_sig, &msg[..MSG1_BEFORE_MAC1], &m1),
        "mac1 over framing+crypto body"
    );
...
fn msg2_layout_carries_g2_fixed_index_semantics() {
    ...
    let mut init = Initiator::new(i_static, r_static.public);
    init.write_initiation(&mut msg1, 0xA70F1E, 42, &r_sig, &[0u8; MAC_BYTES])
        .unwrap();
    ...
    assert_eq!(
        info.initiator_static_pub, i_static.public,
        "s: initiator static decrypted"
    );

    let mut msg2 = [0u8; MSG2_SIZE];
    resp.write_response(&mut msg2, 0x0000_0000, 0xA70F1E, &r_sig, &[0u8; MAC_BYTES])
        .unwrap();
    assert_eq!(msg2[0], 2, "type byte");
    assert_eq!(
        &msg2[4..8],
        &0x0000_0000u32.to_be_bytes(),
        "sender = responder slot (G2: was swapped)"
    );
    assert_eq!(
        &msg2[8..12],
        &0xA70F1Eu32.to_be_bytes(),
        "receiver = echo of initiator index"
    );
    assert_eq!(&msg2[76..92], &[0u8; 16][..], "mac2 zeros");
    let m1: [u8; 16] = msg2[60..76].try_into().unwrap();
    assert!(
        verify_mac1(&r_sig, &msg2[..MSG2_BEFORE_MAC1], &m1),
        "mac1 over [0..60)"
    );
}
...
fn roundtrip_transcript_and_cross_keys() {
    ...
    let mut msg1 = [0u8; MSG1_SIZE];
    let mut init = Initiator::new(i_static, r_static.public);
    init.write_initiation(&mut msg1, 1, 7, &r_sig, &[0u8; MAC_BYTES])
        .unwrap();

    let mut resp = Responder::new(r_static);
    let _info = resp.read_initiation(&msg1, &r_sig).unwrap();
    let mut msg2 = [0u8; MSG2_SIZE];
    resp.write_response(&mut msg2, 9, 1, &r_sig, &[0u8; MAC_BYTES])
        .unwrap();
    ...
    assert_eq!(
        hi.handshake_hash, hr.handshake_hash,
        "transcript hash identical"
    );
    assert_eq!(
        hi.send_key, hr.recv_key,
        "initiator send = responder recv (c1)"
    );
    assert_eq!(
        hi.recv_key, hr.send_key,
        "initiator recv = responder send (c2)"
    );
...
fn be_tr_04_mac1_fails_before_any_curve_work() {
    ...
    let mut msg1 = [0u8; MSG1_SIZE];
    let mut init = Initiator::new(i_static, r_static.public);
    init.write_initiation(&mut msg1, 1, 1, &r_sig, &[0u8; MAC_BYTES])
        .unwrap();
    ...
    init2
        .write_initiation(&mut msg1b, 1, 1, &r_sig, &[0u8; MAC_BYTES])
        .unwrap();
    let mut resp2 = Responder::new(r_static);
    resp2.read_initiation(&msg1b, &r_sig).unwrap();
    let mut msg2 = [0u8; MSG2_SIZE];
    resp2
        .write_response(&mut msg2, 1, 1, &r_sig, &[0u8; MAC_BYTES])
        .unwrap();
    msg2[61] ^= 0xFF;
...
fn tampered_ciphertext_is_decrypt_failed() {
    ...
    let mut resp = Responder::new(r_static);
    assert_eq!(
        resp.read_initiation(&msg1, &r_sig),
...
fn transport_nonce_composition_big_endian_everywhere() {
    ...
    assert_eq!(
        transport_nonce(0x0102_0304_0506_0708),
        [0, 0, 0, 0, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]
    );
    assert_eq!(
        transport_nonce(u64::MAX),
...
fn hmac_key_exactly_64_bytes_uses_direct_path() {
    ...
    let mut k64 = [0u8; 64];
    for (i, b) in k64.iter_mut().enumerate() {
        *b = i as u8;
    }
