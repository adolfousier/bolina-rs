...
use bolina::state::ledger::{GrantLedger, GRANT_ID_LEN, MAX_LIVE, SIG_PUBKEY_LEN};
...
fn ledger_consumed_max_live_boundary() {
    ...
    for i in 0..MAX_LIVE {
        ...
        let now_ms = 0;
        ledger
            .commit_consumed(&grant_id, expiry_ms, now_ms)
            .unwrap();
    }
...
fn ledger_revoked_max_live_boundary() {
    ...
    for i in 0..MAX_LIVE {
        ...
        ledger
            .commit_revocation(&sig_pubkey, cert_expiry_ms)
            .unwrap();
    }
