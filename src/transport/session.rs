...
impl CipherState {
    ...
    pub fn seal(&mut self, out: &mut [u8], plaintext: &[u8], ad: &[u8]) -> Result<()> {
        if self.counter >= REKEY_AFTER_MESSAGES {
...
impl Session {
    ...
    pub fn due_for_rekey(&self, now_ms: u64) -> bool {
        self.send.counter >= REKEY_AFTER_MESSAGES || now_ms >= self.key_epoch_ms + REKEY_AFTER_MS
    }
    ...
    pub fn seal(&mut self, out: &mut [u8], plaintext: &[u8]) -> Result<usize> {
        ...
        self.send
            .seal(&mut out[HEADER_SIZE..total], plaintext, &header)?;
...
impl SessionTable {
    pub fn new() -> Self {
        Self {
            slots: std::array::from_fn(|_| Session::new()),
        }
    }

    pub fn lookup(&mut self, local_index: u32) -> Option<&mut Session> {
        if (local_index as usize) >= MAX_SESSIONS {
            return None;
        }
    ...
    pub fn admit(
        ...
        let slot = &mut self.slots[local_index as usize];
        if slot.in_use {
            return Err(TransportError::SlotFull);
        }
        slot.local_index = local_index;
        slot.peer_index = peer_index;
        slot.send.zero();
        slot.recv.zero();
        slot.send.key = send_key;
        slot.recv.key = recv_key;
