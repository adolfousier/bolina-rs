...
use bolina::transport::noise::{
    transport_nonce, HandshakeResult, Initiator, KeyPair, MSG1_SIZE, MSG2_SIZE,
    OFF2_RECEIVER_INDEX, OFF2_SENDER_INDEX,
};
...
fn env(k: &str) -> String {
    std::env::var(k).unwrap_or_else(|_| panic!("env {k} unset"))
}
...
fn seal_t4(out_key: &[u8; 32], counter: u64, pt: &[u8]) -> Vec<u8> {
    ...
    let ct = cipher
        .encrypt(
            Nonce::from_slice(&transport_nonce(counter)),
...
fn main() {
    ...
    let token = std::fs::read_to_string(env("ITO_TOKEN_FILE"))
        .expect("token file")
        .trim()
        .to_string();
    ...
    assert_eq!(
        our_cert_rec.sig_pubkey, &our_sig_pub,
        "cert/signing key mismatch"
    );
    ...
    initiator
        .write_initiation(&mut msg1, our_index, now_ms(), &daemon_sig_pub, &[0; 16])
        .expect("msg1");
    if std::env::var("ITO_DEBUG").is_ok() {
        eprintln!("MSG1={}", msg1.map(|b| format!("{b:02x}")).concat());
    }
    ...
    assert_eq!(
        &msg2[OFF2_RECEIVER_INDEX..OFF2_RECEIVER_INDEX + 4],
        &our_index.to_be_bytes(),
        "G2 echo index"
    );
    let daemon_slot = u32::from_be_bytes(
        msg2[OFF2_SENDER_INDEX..OFF2_SENDER_INDEX + 4]
            .try_into()
            .unwrap(),
    );
    initiator
        .read_response(&msg2, &daemon_sig_pub)
        .expect("msg2 verify");
    ...
    assert_eq!(
        dcert.sig_pubkey, &daemon_sig_pub,
        "daemon cert is the daemon"
    );
    let mut bmsg = vec![0x05u8]; // DOMAIN_BINDING
    bmsg.extend_from_slice(&hs.handshake_hash);
    VerifyingKey::from_bytes(&daemon_sig_pub)
        .unwrap()
        .verify(&bmsg, &daemon_pt[2 + cert_len..].try_into().unwrap())
        .expect("daemon binding sig");
    ...
    let want = format!(
        "/v1/intents/{}",
        intent_id.map(|b| format!("{b:02x}")).concat()
    );
    ...
    assert!(
        resp.contains("pending"),
        "intent state not pending: {}",
